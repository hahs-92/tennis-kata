public class TennisGame2 implements TennisGame
{
    ...
    private String[] getResWhenP1GreaterThanZeroAndP2EqualsToZero(int p1) {
        if (p1==1)
    ...
    private String[] getResWhenP1GreaterThanP2AndLessThanZero(int point1, int point2, String res1, String res2 ) {
        if (point1 == 2)
            res1 = "Thirty";
        if (point1 == 3)
            res1 = "Forty";
    ...
    private String[] getPiresAndScoreWHenP1orP2GreaterThanZero(int p1point, int p2point,String p1res, String p2res, String score) {
        if (p2point == 0 )
        {
            String[] pires = getResWhenP1GreaterThanZeroAndP2EqualsToZero(p1point);
            return new String[]{pires[0], pires[1], pires[0] + "-" + pires[1]};
        }

        if (p1point == 0 )
        {
            String[] pires = getResWhenP1GreaterThanZeroAndP2EqualsToZero(p2point);
    ...
    private String[] getPiresAndScoreWhenP1orP2LessThanFour(int p1point, int p2point, String p1res, String p2res, String score) {
        if (p1point > p2point)
        {
            String[] listRes = getResWhenP1GreaterThanP2AndLessThanZero(p1point, p2point, p1res, p2res);
            return new String[]{listRes[0], listRes[1], listRes[0] + "-" + listRes[1] };
        }
        if(p2point > p1point) {
            String[] listRes = getResWhenP1GreaterThanP2AndLessThanZero(p2point, p1point, p2res, p1res);
    ...
    private boolean isPGreaterThan4AndSubstractP1AndP2GreatherThan2(int p1point, int p2point) {
        return isSubstractGreatherThan2(P1point, P2point) || isSubstractGreatherThan2(P2point, P1point);
    }

    private boolean isP1OrP2GreaterThan4(int p1point, int p2point) {
        return p1point < 4 || p2point < 4;
    }

    private boolean isP1orP2GreaterThanZero(int p1point, int p2point) {
        return p1point > 0 || p2point > 0;
    }

    private boolean isP1AndP2Equal(int p1point, int p2point) {
    ...
    public String getScore(){
        ...
        if(isP1AndP2Equal(P1point, P2point)) {
            score =  getScoreWhenPointsAreEquals(P1point);
        }

        if(isP1orP2GreaterThanZero(P1point, P2point)) {
            String[] resp = getPiresAndScoreWHenP1orP2GreaterThanZero(P1point, P2point,P1res, P2res, score);
            P1res = resp[0];
            P2res = resp[1];
            score = resp[2];
        }

        if(isP1OrP2GreaterThan4(P1point, P2point)) {
            String[] res = getPiresAndScoreWhenP1orP2LessThanFour(P1point,P2point,P1res,P2res, score);
            P1res = res[0];
            P2res = res[1];
            score = res[2];
        }
        ...
        if(isPGreaterThan4AndSubstractP1AndP2GreatherThan2(P1point, P2point) ) {
